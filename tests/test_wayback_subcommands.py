...
def test_check_success_and_available():
    ...
    assert answer == 'http://web.archive.org/web/20200903230055/https://www.whitehouse.gov/issues/immigration/'
    assert data['url'] == target_url # TODO: not sure if this is guranteed, if target_url ends up being redirected??
...
def test_snapshot_submit_request(session):
    ...
    resptext = EXAMPLES_DIR.joinpath('job-save-success/submit-response.html').read_text()
    ...
    responses.add('POST',
        ...
        status=200,
        match=[
            responses.urlencoded_params_matcher({'url': target_url, 'capture_all': 'on'})
        ]
    )
...
def test_snapshot_submit_request_not_ok(session):
    ...
    with pytest.raises(WaybackSubmitError) as err:
        resp = wb.submit_snapshot_request(session, target_url, headers={})
    assert f'Server status was NOT OK; returned 503 for: {save_url}' in err.value.args[0]
...
def test_snapshot_successful():
    ...
    assert answer == wb.BASE_DOMAIN + '/web/' + data['last_job_status']['timestamp'] + '/' + target_url
